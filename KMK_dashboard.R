...
Etalase_news_topiclist_tbl_trim$topics <- tolower(Etalase_news_topiclist_tbl_trim$topics)
Etalase_news_topiclist_tbl_trim <- Etalase_news_topiclist_tbl_trim %>%
  mutate(topics = str_replace_all(topics, ' ', '-')) %>%
  mutate(tag_url = paste0(liputan6_tag, topics)) %>%
...
googlenews_scraped = Etalase_news_topiclist_tbl_trim$googlenews_url %>%
