...
visitDataFinal$vid <- as.numeric(visitDataFinal$vid)
transData$sid <- as.character(transData$sid)
...
transID <- as.character(combineData$transID)
...
combineDataFill <- transform(combineData, transFill = stack(lapply(unstack(combineData, transID ~ vid), na.locf, na.rm = FALSE)))
...
combineDataFill_1 <- transform(combineDataFill, transDateFill = stack(lapply(unstack(combineDataFill, transDate ~ vid), na.locf, na.rm = FALSE)))
...
transInfo2$vid <- as.character(transInfo2$vid)
