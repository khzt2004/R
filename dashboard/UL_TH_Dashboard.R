...
campaign_commercial_perf_pivot_query <- paste0("SELECT *
                           FROM (
                           SELECT
...
target_data_ASP_CR <- target_data %>%
  ...
  select(1:6, Sub_Category = "Category", 
         NMV_Target = 'NMV', 
         Item_Target = 'Item', PV_Target = 'PV')

ASP_CR_combined_master <- actuals_data %>%
  left_join(target_data_ASP_CR, by =c("Year", "Country", "Month", "Channel", 
                                       "Sub_Category", 
                                      "Partnership")) %>%
  select("Date_of_Week","Year","Month", "Week", "Channel","Country", 
         "Partnership", "NMV", "Item", "ASP", "PV", "CR", "NC", "Category", 
         "Sub_Category", 
         "NMV_Target",
...
combined_Target_Ach_master <- shopee_lazada_master %>%
  ...
  select("Date_of_Week","Year","Month", "Week", "Channel","Country",
         "Partnership", "Metric", "Achieved", "Category", "Sub_Category", "Target",
         "pct_Achieved") %>%
  mutate(Category = case_when(is.na(Category) ~ "Others",
...
Campaign_commercial_reportName <- 'campaign_commercial_perf_pivot_table'
